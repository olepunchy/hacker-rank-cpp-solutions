...
//
// We then process each query and print key=value if the queried  is found in
...
int GetNumberOfContacts() {
  cout << "Number of contacts to enter: ";
  string numberOfContacts;
  getline(cin, numberOfContacts);
  return stoi(numberOfContacts);
}

void AddContacts(map<string, int> &contacts, int numberOfContacts) {
  for (int index = 0; index < numberOfContacts; index++) {
    string contactName, phoneNumberString;
    cout << "Enter contact name and phone number separated by space: ";
    cin >> contactName >> phoneNumberString;
    int phoneNumber = stoi(phoneNumberString);

    contacts[contactName] = phoneNumber;
  }
}

void SearchLoop(map<string, int> &contacts) {
  string name;
  while (cin >> name) {
    if (contacts.find(name) != contacts.end()) {
      cout << name << "=" << contacts.find(name)->second << "\n";
    } else {
      cout << "Not found\n";
    }
  }
...
int main() {
  ...
  AddContacts(contacts, numberOfContacts);
